...
首页和观察台分别挂载同一个场景模块，路由互斥，正常页面仅一个 WebGL 上下文。首页只展示星轨共振；实验入口使用 DOM，不再创建三个微场景、行星或连接光路。

## 场景与动效

- 首页：斜前方的圆环星轨共振，以及三个对齐的简洁实验入口。移除珠光核心切换、装饰模型和展示调试控件。
...
- ResizeObserver 重新构图，DPR 设置上限。动画不逐帧更新整个 React 应用，界面不显示渲染监测数字。
- 卸载取消 RAF、监听器和 observer，并释放几何、材质、环境贴图与 renderer。
- 动态导入 Three.js；未加载完即可点击实验入口。首页图形不可用时隐藏画布，观察台使用 CSS 备用装置，浏览器上下文恢复时自动重绘。
- 没有展示备用视图或重试 3D 按钮；实际 context-loss 恢复分支尚未通过强制 GPU 丢失验证。

渲染统计仅为内部采样，不再出现在产品界面。没有全屏 Bloom、景深、多层实时阴影或视频录制。
...
| visual/Observatory.tsx             | 首页构图、真实 DOM 入口与自动降级        |
...
| 视频区间 | 观察到的特征 | 程序化呈现 |
| --- | --- | --- |
| 全程（包括首次加载） | 圆环清楚、光轴向右下近处延伸；符号沿环与谱线移动 | 只保留斜前方方向，角度小幅缓变；圆环保持分离，串珠半径轻微脉动，不再使用侧视开场或切镜 |
...
镜头方向在不依赖 React 的 `resonanceChoreography.ts` 中定义。装置自动运行，始终保持斜前方，不控制实验或发起模型请求。减少动态与后台暂停仍然有效。

复用 ObservatoryScene 的离屏/后台暂停及资源清理，质量切换会释放旧几何体与材质。首页只保留星轨共振并压暗局部背景。没有读取音频、麦克风或模型数据。该效果是根据录屏观察实现的近似，不是原视频源码或逐帧精确复刻。
