...
- npm，以及支持 Canvas 和 CSS 3D 的现代浏览器。
...
首页和 Tetris 工作区已实现 CSS 3D + Canvas 样板：具有光照的行星与核心球体、多层轨道、立体实验入口、带厚度与底座的棋盘、倒角方块、双层投影和落地/消行扩散。

**Jev AI Core** 在待机时慢转，随实际请求、校验和执行加速，收到结果后脉冲，发生异常时变色并显示文字。首页核心为待机装置，不生成虚假的运行状态。

顶部的空间视觉质量提供三档：

| 档位     | 行为                           |
| -------- | ------------------------------ |
| 标准空间 | 较少星点和轨道，保留体积与光照 |
| 增强空间 | 更多轨道、星点和轻微鼠标视差   |
| 减少动态 | 停止装饰动画，保留静态立体结构 |

系统“减少动态效果”优先。低核数或低内存设备会限制为标准档；这是设备提示降级，不是 GPU 跑分。后台暂停装饰动画，离开视口的核心暂停轨道。显示设置不改变实验节奏或调用预算。

路径和工作台已复用设备外框、观察台及首页微场景，其实验内部完整 3D 改造尚未推广。表单和 JSON 保持清晰的二维排版。
...
技术栈：React、TypeScript、Vite、Tailwind CSS、Canvas、CSS 3D；Node.js、Express、Zod、SQLite；Vitest 与 Testing Library。
...
最近一次已记录验证：**8 个测试文件、85 项测试全部通过，生产构建通过**。桌面与 390×844 手机视口已检查首页、棋盘、核心状态及减少动态模式。详细范围见 [验证记录](docs/VERIFICATION.md)。
...
已实现统一平台、三个可运行实验、供应商管理、观察台、会话记录和脱敏导出，以及首页与 Tetris 的 3D 样板。
