...
首页默认展示实时 3D「星轨共振」：七层空心圆环沿纵深排列，中央是串珠状粒子光轴，外围三组五线谱承载沿轨道流动的音符、星星和空心几何符号。镜头从首次加载开始始终保持斜前方方向，仅有轻微角度变化；重播与拖动时间也不会切回侧视。底部支持暂停、拖动参考时间与重播，也可切换到「珠光核心」。这些是装饰视觉，不代表模型数据，也不读取麦克风；减少动态模式静态呈现，离屏或后台暂停动画。
...
- npm，以及支持 Canvas、CSS 3D 和 WebGL 2 的现代浏览器（WebGL 不可用时仍可使用备用视图）。
...
- **首页**：Three.js 实时渲染珠光核心、分段刻度轨道、行星，以及方块舱、导航沙盘、决策节点盘。所有首页装置共用一个 WebGL 上下文；固定的中文按钮提供真实实验入口，场景本身是标明用途的装饰预览。
...
首页 **渲染监测** 显示实际采样 FPS、CPU 渲染提交时间、draw calls、三角形和 DPR。它不包含模型耗时，也不是 GPU 基准成绩。未实现景深和全屏 Bloom，避免文本模糊与无差别高采样。

WebGL 不可用、资源加载失败、上下文中断时使用同风格 CSS 备用装置，实验入口仍然可用；可主动切换备用视图或重试。表单、模型配置和长文本始终使用 DOM，不随鼠标倾斜。
...
最近一次已记录验证：**10 个测试文件、94 项测试全部通过，生产构建通过**。已检查 1440、1024、390 CSS 像素视口，以及请求、错误兜底、历史、减少动态和备用视图。详细范围见 [验证记录](docs/VERIFICATION.md)。
